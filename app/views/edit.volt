                            ...
                            <form role="form" action="/players/edit/{{ player.pid }}" method="post">
                                <div class="tab-pane active" id="m_user_profile_tab_1">
                                    <div class="m-portlet__body">
                                        ...
                                        {% if user.panelLevel >= constant('Players::SA') %}
                                            <div class="form-group m-form__group row">
                                        ...
                                        {% if user.panelLevel >= constant('Players::MODERATOR') %}
                                            <div class="form-group m-form__group row">
                                                ...
                                                <div class="form-group form-md-line-input form-md-floating-label has-info">
                                                    <select class="form-control edited" id="player_cop" name="cop">
                                                        ...
                                                        {% if user.panelLevel >= constant('Players::ADMINISTRATOR') %}<option value="7" {% if player.coplevel == 7 %}selected{% endif %}>State Command</option>{% endif %}
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="form-group m-form__group row">
                                        ...
                                        {% endif %}
                                        {% if user.panelLevel >= constant('Players::ADMINISTRATOR') %}
                                            <div class="form-group m-form__group row">
                                        ...
                                        {% if user.panelLevel >= constant('Players::SM') %}
                                            <div class="form-group m-form__group row">
                                                <label for="example-text-input" class="col-2 col-form-label">Panel Level (NOTE THIS IS FOR GIVING PEOPLE ACCESS TO THE PANEL)</label>
                                                <div class="col-7">
                                                    <select class="form-control edited" id="player_admin" name="panel">
                                                        <option value="0"  {% if player.panelLevel == 0 %}selected{% endif %}>No Access</option>
                                                        <option value="1"  {% if player.panelLevel == 1 %}selected{% endif %}>Moderator</option>
                                                        <option value="2"  {% if player.panelLevel == 2 %}selected{% endif %}>Administrator</option>
                                                        {% if user.panelLevel >= constant('Players::SM') %}<option value="3"  {% if player.panelLevel == 3 %}selected{% endif %}>Senior Administrator</option>{% endif %}
                                                        {% if user.panelLevel >= constant('Players::SM') %}<option value="4"  {% if player.panelLevel == 4 %}selected{% endif %}>Head Administrator</option>{% endif %}
                                                        {% if user.panelLevel >= constant('Players::CM') %}<option value="5"  {% if player.panelLevel == 5 %}selected{% endif %}>Staff Manager</option>{% endif %}
                                                        {% if user.panelLevel >= constant('Players::CD') %}<option value="6"  {% if player.panelLevel == 6 %}selected{% endif %}>Community Manager</option>{% endif %}
                                                        {% if user.panelLevel >= constant('Players::FOUNDER') %}<option value="7"  {% if player.panelLevel == 7 %}selected{% endif %}>Community Director</option>{% endif %}
                                                        {% if user.panelLevel >= constant('Players::FOUNDER') %}<option value="8" {% if player.panelLevel == 8 %} selected {% endif %}>Founder</option>{% endif %}
                                                    </select>
                                                </div>
                                            </div>
                                        {% endif %}
